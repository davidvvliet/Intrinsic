...
export function ThreeJSGlobeWithDots({ 
  ...
  }, [size, color, speed, showWireframe]); // Removed dots and onDotClick from dependencies
  ...
  useEffect(() => {
    ...
    // Process dots (strings or PersonaDot objects)
    const dotsToProcess = !dots || dots.length === 0 ? [] : dots;
    ...
    dotsToProcess.forEach((dot, index) => {
      ...
      // Parse coordinate string or use PersonaDot object
      if (typeof dot === 'string') {
        const match = dot.match(/\(([^,]+),([^)]+)\)/);
        if (!match) return;
      ...
      const position = latLonToVector3(lat, lon, globeRadius + 0.02);
      ...
      const dotData = typeof dot === 'string' ? { id: index, lat, lon, color, size: 2, opacity } : dot;
      dotMesh.userData = { dot: dotData };
